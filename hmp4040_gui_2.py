...
class HMP4040GUI(tk.Tk):
    ...
    def _build_ui(self):
        ...
        self.device_name_entry = ttk.Entry(conn, textvariable=self.device_name_var, width=14)
        self.device_name_entry.grid(row=0, column=5, padx=6, pady=6, sticky="w")
        # 이름이 바뀌면 바로 IDN 라벨 갱신
        try:
            self.device_name_var.trace_add("write", lambda *args: self._update_idn_label())
        except Exception:
            # Python 3.7 호환 (trace_add 미지원 시)
            self.device_name_var.trace("w", lambda *args: self._update_idn_label())

        # IDN 라벨은 한 칸 오른쪽으로 이동
    ...
    def _update_idn_label(self):
        """사용자 이름과 IDN을 합쳐 라벨을 갱신"""
        user_name = (self.device_name_var.get() or "").strip()
        if self.last_idn:
            if user_name:
                self.idn_label.config(text=f"{user_name} | [IDN] {self.last_idn}")
            else:
                self.idn_label.config(text=f"[IDN] {self.last_idn}")
        else:
            if user_name:
    ...
    def _do_toggle_output(self, ch, state):
        try:
            self.inst.write(f"INSTrument:NSELect {ch)")
            # <-- Fix typo above if you copy by hand; correct line is below -->
        except Exception:
            pass  # placeholder to keep structure; real write is next line
        try:
            self.inst.write(f"INSTrument:NSELect {ch}")
