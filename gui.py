# general_scpi_gui.py
# Row-click activation + model-aware PSU tab + PSU label display buttons
# Supports: E3631A (P6V/P25V/N25V), E3633A (OUT, single-output dual-range), HMP4040 (1..4), HMP4030 (1..3), HM8143 (U1/U2)
# Added: DMM/SMU/PSU "Show Label"/"Clear" buttons
# Updated: Power Supply control UI moved to a dedicated Notebook tab
...
class GeneralSCPIGUI(tk.Tk):
    ...
    def _build_ui(self):
        ...
        # ----- Notebook Tabs (Log / Power Supply) -----
    ...
    def _set_psu_tab_enabled(self, enabled: bool):
        try:
            self.notebook.tab(self.psu_tab, state=("normal" if enabled else "disabled"))
        except Exception:
            pass  # some Tk builds may not support per-tab state
    ...
    def _update_psu_panel(self):
        ...
        if model == "HMP4040":
            ...
            chs = ["OUT"]  # single output; keep uniform UI
        elif model == "HM8143":
            chs = ["U1", "U2"]   # HM8143: only U1/U2 support remote V/I set/query
        else:
            # not a recognized PSU -> disable tab
            self.psu_channel_combo["values"] = []
            self._set_psu_tab_enabled(False)
            return

        # recognized PSU -> enable tab, configure channels
    ...
    def psu_show_label_text(self):
        try:
            if not self._check_connected(): return
            # Check if active is a recognized PSU
            idn = self._active_idn()
            model = self._detect_model(idn)
            if not model:
            ...
            # Generate label text from Devices table (expects Type 'ps')
            t, n = self._active_type_num()
            if not t:
            ...
            # If PSU doesn't support label display, warn
            ...
            # Keysight E3631A/E3633A: try typical display text sequences
            sequences = [
