...
class GeneralSCPIGUI(tk.Tk):
    def __init__(self):
        ...
        # Devices table state
        self.device_rows = []            # list of row dicts: {"resource": str, "label_var": tk.StringVar}
    ...
    def _build_devices_table_headers(self):
        ...
        headers = ["#", "Label", "VISA Resource", "IDN"]
        for c, text in enumerate(headers):
            lbl = tk.Label(self.device_table, text=text, borderwidth=1, relief="solid", padx=6, pady=4, anchor="w", font=("TkDefaultFont", 9, "bold"))
            lbl.grid(row=0, column=c, sticky="nsew")

        # Column expand
        for c, weight in enumerate([0, 1, 2, 3]):
    ...
    def _refresh_devices_table(self):
        ...
        for r, resource_key in enumerate(sorted(self.sessions.keys()), start=1):
            ...
            idn = info.get("idn", "")
            label_var = tk.StringVar(value=info.get("label", ""))

            # index cell
            tk.Label(self.device_table, text=str(r), borderwidth=1, relief="solid", padx=4, pady=2, anchor="w").grid(row=r, column=0, sticky="nsew")

            # label cell (editable)
            e = tk.Entry(self.device_table, textvariable=label_var, borderwidth=1, relief="solid")
            e.grid(row=r, column=1, sticky="nsew")

            # resource cell
            tk.Label(self.device_table, text=resource_key, borderwidth=1, relief="solid", padx=6, pady=2, anchor="w").grid(row=r, column=2, sticky="nsew")

            # idn cell
            tk.Label(self.device_table, text=idn, borderwidth=1, relief="solid", padx=6, pady=2, anchor="w").grid(row=r, column=3, sticky="nsew")

            # keep bindings
            def _on_change(*_, rk=resource_key, var=label_var):
                self.sessions[rk]["label"] = var.get().strip()
                if rk == self.connected_resource:
                    self._update_idn_banner()
                self._check_labels_filled()

            label_var.trace_add("write", _on_change)
            self.device_rows.append({"resource": resource_key, "label_var": label_var})
    ...
    def create_scripts(self):
        ...
        for row in self.device_rows:
            ...
            if not label:
                messagebox.showerror("Invalid label", "All labels must be non-empty and valid.")
                return
            labels.append(label)
