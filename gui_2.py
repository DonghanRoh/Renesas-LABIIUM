# general_scpi_gui.py
# Row-click activation + model-aware PSU/DMM/SMU/FGEN tabs + Label display buttons
# Supports:
...
#   - DMM/SMU/FGEN 기본 조작 UI와 안전한 SCPI 시퀀스 시도(fallback sequences)
#   - DMM/SMU/PSU "Show Label"/"Clear" 버튼 유지
#
# Updated:
#   - Power Supply control UI는 Notebook의 전용 탭
#   - Row click로 활성 장치 전환 시 모든 탭의 enable 상태 갱신
...
class GeneralSCPIGUI(tk.Tk):
    def __init__(self):
        ...
        # DMM state
        self.dmm_model_label_var = tk.StringVar(value="")
        self.dmm_mode_var = tk.StringVar(value="VOLT:DC")
        self.dmm_value_var = tk.StringVar(value="")

        # SMU state
        self.smu_model_label_var = tk.StringVar(value="")
        self.smu_sourcemode_var = tk.StringVar(value="VOLT")  # VOLT or CURR
        self.smu_level_var = tk.StringVar(value="")
        self.smu_meas_v_var = tk.StringVar(value="")
        self.smu_meas_i_var = tk.StringVar(value="")

        # FGEN state
        self.fgen_model_label_var = tk.StringVar(value="")
        self.fgen_waveform_var = tk.StringVar(value="SIN")  # SIN,SQU,RAMP,PULSE,NOIS,DC
        self.fgen_freq_var = tk.StringVar(value="1000")     # Hz
        self.fgen_amp_var = tk.StringVar(value="1.0")       # Vpp
        self.fgen_offset_var = tk.StringVar(value="0.0")    # V
    ...
    def _build_ui(self):
        ...
        # DMM/SMU/PSU label controls (existing)
        ...
        # ----- Notebook Tabs (Log / Power Supply / Multi Meter / Source Monitor Unit / Function Generator) -----
        ...
        # PSU tab
        self.psu_tab = ttk.Frame(self.notebook)
        self.notebook.add(self.psu_tab, text="Power Supply")
        self._build_psu_controls(parent=self.psu_tab)  # builds into PSU tab

        # DMM tab
        self.dmm_tab = ttk.Frame(self.notebook)
        self.notebook.add(self.dmm_tab, text="Multi Meter")
        self._build_dmm_controls(parent=self.dmm_tab)

        # SMU tab
        self.smu_tab = ttk.Frame(self.notebook)
        self.notebook.add(self.smu_tab, text="Source Monitor Unit")
        self._build_smu_controls(parent=self.smu_tab)

        # FGEN tab
        self.fgen_tab = ttk.Frame(self.notebook)
        self.notebook.add(self.fgen_tab, text="Function Generator")
        self._build_fgen_controls(parent=self.fgen_tab)
        ...
        self._update_device_specific_tabs()

    # ---------- Generic tab state helper ----------
    def _set_tab_enabled(self, tab, enabled: bool):
        try:
            self.notebook.tab(tab, state=("normal" if enabled else "disabled"))
        except Exception:
            pass

    # ---------- PSU Controls (in the PSU Tab) ----------
    def _build_psu_controls(self, parent):
        psu_frame = ttk.LabelFrame(parent, text="Power Supply Controls")
        psu_frame.pack(fill="x", padx=10, pady=10)
    ...
    def _set_psu_tab_enabled(self, enabled: bool):
        self._set_tab_enabled(self.psu_tab, enabled)
    ...
    def _update_psu_panel(self):
        """Update PSU tab contents and enablement based on the active device."""
        if not self.inst or not self.connected_resource or self.connected_resource not in self.sessions:
            self.psu_model_label_var.set("(No PSU)")
            self.psu_channel_combo["values"] = []
            self._set_psu_tab_enabled(False)
            return
        ...
        self._set_psu_tab_enabled(True)
        self.psu_channel_combo["values"] = chs
        if len(chs) == 1:
            self.psu_channel_combo.configure(state="disabled")
            self.psu_channel_var.set(chs[0])
        else:
            self.psu_channel_combo.configure(state="readonly")
            if not self.psu_channel_var.get() or self.psu_channel_var.get() not in chs:
                self.psu_channel_var.set(chs[0])

    # ---------- DMM/SMU/FGEN model detection ----------
    @staticmethod
    def _is_supported_dmm(idn: str) -> bool:
        s = (idn or "").upper()
        # avoid false positive for HMP4040 PSU while honoring "4040"
        if "HMP4040" in s:
            s = s.replace("HMP4040", "")
    ...
    def _build_dmm_controls(self, parent):
        frame = ttk.LabelFrame(parent, text="Multi Meter Controls")
        frame.pack(fill="x", padx=10, pady=10)

        ttk.Label(frame, text="Model:").grid(row=0, column=0, padx=6, pady=8, sticky="e")
        ttk.Label(frame, textvariable=self.dmm_model_label_var).grid(row=0, column=1, padx=(0, 12), pady=8, sticky="w")

        ttk.Label(frame, text="Mode:").grid(row=0, column=2, padx=6, pady=8, sticky="e")
        self.dmm_mode_combo = ttk.Combobox(frame, textvariable=self.dmm_mode_var, state="readonly",
                                           values=["VOLT:DC", "VOLT:AC", "CURR:DC", "RES", "FREQ"], width=12)
        self.dmm_mode_combo.grid(row=0, column=3, padx=(0, 12), pady=8, sticky="w")

        ttk.Button(frame, text="Set Mode", command=self.dmm_set_mode).grid(row=0, column=4, padx=6, pady=8)
        ttk.Button(frame, text="Query Reading", command=self.dmm_query_measurement).grid(row=0, column=5, padx=6, pady=8)

        ttk.Label(frame, text="Reading:").grid(row=1, column=0, padx=6, pady=8, sticky="e")
        ttk.Entry(frame, textvariable=self.dmm_value_var, width=18, state="readonly").grid(row=1, column=1, padx=(0,12), pady=8, sticky="w")

        for c, w in enumerate([0,1,0,1,0,0]):
            frame.grid_columnconfigure(c, weight=w)

    def _update_dmm_panel(self):
        enabled = False
        model_text = "(No DMM)"
        if self.inst and self.connected_resource in self.sessions:
            idn = self.sessions[self.connected_resource].get("idn", "")
            if self._is_supported_dmm(idn):
                enabled = True
                model_text = (idn or "").strip()
        self.dmm_model_label_var.set(model_text)
        self._set_tab_enabled(self.dmm_tab, enabled)

    def dmm_set_mode(self):
        try:
            if not self._check_connected(): return
            idn = self._active_idn().upper()
            mode = self.dmm_mode_var.get().upper()
            # Try robust sequence: CONF first, then FUNC '...'
            sequences = [
                [f"CONF:{mode}"],
                [f"FUNC '{mode}'"],
                [f"FUNC {mode}"],
            ...
            self._log(f"[DMM] Set Mode -> {mode} | IDN={idn}")
        except Exception as e:
            messagebox.showerror("DMM Set Mode failed", str(e))

    def dmm_query_measurement(self):
        try:
            if not self._check_connected(): return
            idn = self._active_idn().upper()
            mode = self.dmm_mode_var.get().upper()

            # Preferred measure command for modern DMMs
            measure_cmd = f"MEAS:{mode}?"
            # Fallback list
            candidates = [measure_cmd, "READ?", "FETCh?", "MEAS?"]
            ...
            for cmd in candidates:
                try:
                    ...
                    if resp:
                        val = self._extract_number(resp)
                        self.dmm_value_var.set(val)
                        self._log(f"[DMM] {cmd} -> {resp}")
                        self._drain_error_queue("[DMM]")
                        return
                except Exception as e:
                    last_err = e
    ...
    # ---------- SMU tab ----------
    def _build_smu_controls(self, parent):
        frame = ttk.LabelFrame(parent, text="Source Monitor Unit Controls")
        frame.pack(fill="x", padx=10, pady=10)

        ttk.Label(frame, text="Model:").grid(row=0, column=0, padx=6, pady=8, sticky="e")
        ttk.Label(frame, textvariable=self.smu_model_label_var).grid(row=0, column=1, padx=(0, 12), pady=8, sticky="w")

        ttk.Label(frame, text="Source Mode:").grid(row=0, column=2, padx=6, pady=8, sticky="e")
        self.smu_mode_combo = ttk.Combobox(frame, textvariable=self.smu_sourcemode_var, state="readonly",
                                           values=["VOLT", "CURR"], width=10)
        self.smu_mode_combo.grid(row=0, column=3, padx=(0,12), pady=8, sticky="w")
        ttk.Button(frame, text="Set Mode", command=self.smu_set_source_mode).grid(row=0, column=4, padx=6, pady=8)

        ttk.Label(frame, text="Level:").grid(row=1, column=0, padx=6, pady=8, sticky="e")
        ttk.Entry(frame, textvariable=self.smu_level_var, width=12).grid(row=1, column=1, padx=(0,12), pady=8, sticky="w")
        ttk.Button(frame, text="Apply Level", command=self.smu_set_level).grid(row=1, column=2, padx=6, pady=8)
        ttk.Button(frame, text="Output ON", command=lambda: self.smu_output(True)).grid(row=1, column=3, padx=6, pady=8)
        ttk.Button(frame, text="Output OFF", command=lambda: self.smu_output(False)).grid(row=1, column=4, padx=6, pady=8)

        ttk.Label(frame, text="Measure V (V):").grid(row=2, column=0, padx=6, pady=8, sticky="e")
        ttk.Entry(frame, textvariable=self.smu_meas_v_var, width=14, state="readonly").grid(row=2, column=1, padx=(0,12), pady=8, sticky="w")
        ttk.Button(frame, text="Query V", command=self.smu_measure_v).grid(row=2, column=2, padx=6, pady=8)

        ttk.Label(frame, text="Measure I (A):").grid(row=3, column=0, padx=6, pady=8, sticky="e")
        ttk.Entry(frame, textvariable=self.smu_meas_i_var, width=14, state="readonly").grid(row=3, column=1, padx=(0,12), pady=8, sticky="w")
        ttk.Button(frame, text="Query I", command=self.smu_measure_i).grid(row=3, column=2, padx=6, pady=8)

        for c, w in enumerate([0,1,0,0,0]):
            frame.grid_columnconfigure(c, weight=w)

    def _update_smu_panel(self):
        enabled = False
        model_text = "(No SMU)"
        if self.inst and self.connected_resource in self.sessions:
            idn = self.sessions[self.connected_resource].get("idn", "")
            if self._is_supported_smu(idn):
                enabled = True
                model_text = (idn or "").strip()
        self.smu_model_label_var.set(model_text)
        self._set_tab_enabled(self.smu_tab, enabled)

    def smu_set_source_mode(self):
        try:
            if not self._check_connected(): return
            mode = self.smu_sourcemode_var.get().upper()
            sequences = [
                [f"SOUR:FUNC {mode}"],
                [f"SOURce:FUNCTION {mode}"],
            ]
            self._dmm_try_sequences(sequences)
            self._drain_error_queue("[SMU]")
            self._log(f"[SMU] Set Source Mode -> {mode}")
        except Exception as e:
            messagebox.showerror("SMU Set Mode failed", str(e))

    def smu_set_level(self):
        try:
            if not self._check_connected(): return
            mode = self.smu_sourcemode_var.get().upper()
            val = float(self.smu_level_var.get())
            if mode == "VOLT":
                sequences = [
                    [f"SOUR:VOLT {val}"],
                    [f"SOURce:VOLTage {val}"],
                ]
            else:
                sequences = [
                    [f"SOUR:CURR {val}"],
                    [f"SOURce:CURRent {val}"],
                ]
            self._dmm_try_sequences(sequences)
            self._drain_error_queue("[SMU]")
            self._log(f"[SMU] Apply Level -> {val} ({mode})")
        except Exception as e:
            messagebox.showerror("SMU Apply Level failed", str(e))

    def smu_output(self, on: bool):
        try:
            if not self._check_connected(): return
            ...
            self._dmm_try_sequences(sequences)
            self._drain_error_queue("[SMU]")
            self._log(f"[SMU] Output -> {val}")
        except Exception as e:
            messagebox.showerror("SMU Output failed", str(e))

    def smu_measure_v(self):
        try:
            if not self._check_connected(): return
            ...
            for cmd in candidates:
                try:
                    resp = self.inst.query(cmd).strip()
                    if resp:
                        self.smu_meas_v_var.set(self._extract_number(resp))
                        self._log(f"[SMU] {cmd} -> {resp}")
                        self._drain_error_queue("[SMU]")
                        return
                except Exception:
                    continue
        ...
        try:
            ...
            for cmd in candidates:
                try:
                    resp = self.inst.query(cmd).strip()
                    if resp:
                        self.smu_meas_i_var.set(self._extract_number(resp))
                        self._log(f"[SMU] {cmd} -> {resp}")
                        self._drain_error_queue("[SMU]")
                        return
                except Exception:
    ...
    # ---------- FGEN tab ----------
    def _build_fgen_controls(self, parent):
        frame = ttk.LabelFrame(parent, text="Function Generator Controls")
        frame.pack(fill="x", padx=10, pady=10)

        ttk.Label(frame, text="Model:").grid(row=0, column=0, padx=6, pady=8, sticky="e")
        ttk.Label(frame, textvariable=self.fgen_model_label_var).grid(row=0, column=1, padx=(0,12), pady=8, sticky="w")

        ttk.Label(frame, text="Waveform:").grid(row=0, column=2, padx=6, pady=8, sticky="e")
        self.fgen_wave_combo = ttk.Combobox(frame, textvariable=self.fgen_waveform_var, state="readonly",
                                            values=["SIN","SQU","RAMP","PULSE","NOIS","DC"], width=10)
        self.fgen_wave_combo.grid(row=0, column=3, padx=(0,12), pady=8, sticky="w")

        ttk.Label(frame, text="Freq (Hz):").grid(row=1, column=0, padx=6, pady=8, sticky="e")
        ttk.Entry(frame, textvariable=self.fgen_freq_var, width=12).grid(row=1, column=1, padx=(0,12), pady=8, sticky="w")

        ttk.Label(frame, text="Amp (Vpp):").grid(row=1, column=2, padx=6, pady=8, sticky="e")
        ttk.Entry(frame, textvariable=self.fgen_amp_var, width=10).grid(row=1, column=3, padx=(0,12), pady=8, sticky="w")

        ttk.Label(frame, text="Offset (V):").grid(row=1, column=4, padx=6, pady=8, sticky="e")
        ttk.Entry(frame, textvariable=self.fgen_offset_var, width=10).grid(row=1, column=5, padx=(0,12), pady=8, sticky="w")

        ttk.Button(frame, text="Apply", command=self.fgen_apply).grid(row=2, column=0, padx=6, pady=8)
        ttk.Button(frame, text="Output ON", command=lambda: self.fgen_output(True)).grid(row=2, column=1, padx=6, pady=8)
        ttk.Button(frame, text="Output OFF", command=lambda: self.fgen_output(False)).grid(row=2, column=2, padx=6, pady=8)

        for c, w in enumerate([0,1,0,1,0,1]):
            frame.grid_columnconfigure(c, weight=w)

    def _update_fgen_panel(self):
        enabled = False
        model_text = "(No FGEN)"
        if self.inst and self.connected_resource in self.sessions:
            idn = self.sessions[self.connected_resource].get("idn", "")
            if self._is_supported_fgen(idn):
                enabled = True
                model_text = (idn or "").strip()
        self.fgen_model_label_var.set(model_text)
        self._set_tab_enabled(self.fgen_tab, enabled)

    def fgen_apply(self):
        try:
            if not self._check_connected(): return
            wave = self.fgen_waveform_var.get().upper()
            freq = float(self.fgen_freq_var.get())
            amp  = float(self.fgen_amp_var.get())
            offs = float(self.fgen_offset_var.get())

            # Try APPLY first (many Keysight gens)
            sequences = [
            ...
            self._dmm_try_sequences(sequences)
            self._drain_error_queue("[FGEN]")
            self._log(f"[FGEN] Apply -> W={wave}, F={freq}, A={amp}, O={offs}")
        except Exception as e:
            messagebox.showerror("FGEN Apply failed", str(e))

    def fgen_output(self, on: bool):
        try:
            if not self._check_connected(): return
            val = "ON" if on else "OFF"
            sequences = [
    ...
    # ---------- PSU label ops ----------
    def _psu_label_supported(self, model: str) -> bool:
        # Only Keysight E3631A/E3633A support DISP:TEXT style label out of the listed PSUs
        return model in ("E3631A", "E3633A")
    ...
    def psu_show_label_text(self):
        try:
            ...
            model = self._detect_model(idn)
            if not model:
                messagebox.showinfo("Not a PSU", "The active device is not a recognized power supply.")
                return

            t, n = self._active_type_num()
            if not t:
                messagebox.showinfo("Missing Type", "Select Type in the Devices table (e.g., ps).")
                return
            label_text = (t + (n if n and n != "No Number" else "")).strip()
            msg = label_text.replace("'", "''")

            if not self._psu_label_supported(model):
                messagebox.showwarning("Label Not Supported",
                                       f"{model} does not support displaying custom text on the front panel.")
                return
    ...
    def psu_clear_label_text(self):
        try:
            ...
            if not self._psu_label_supported(model):
                messagebox.showwarning("Label Not Supported",
                                       f"{model} does not support clearing custom text (label display not supported).")
                return
    ...
    def psu_query_voltage(self):
        try:
            ...
            if model == "HM8143":
                idx = self._hm8143_ch_index(channel)
                resp = self.inst.query(f"RU{idx}").strip()
            else:
                self._psu_select_channel(model, channel)
                if model in ("HMP4040", "HMP4030"):
    ...
    def psu_query_current(self):
        ...
        # (rest of psu_query_current)
            if model == "HM8143":
                idx = self._hm8143_ch_index(channel)
                resp = self.inst.query(f"RI{idx}").strip()
            else:
                self._psu_select_channel(model, channel)
                if model in ("HMP4040", "HMP4030"):
    ...
    def disconnect_current(self):
        try:
            if self.inst and self.connected_resource:
                ...
                self._refresh_devices_table()
                self._set_psu_tab_enabled(False)
                self._set_tab_enabled(self.dmm_tab, False)
                self._set_tab_enabled(self.smu_tab, False)
                self._set_tab_enabled(self.fgen_tab, False)
            else:
    ...
    # ---------- device-specific tabs updater ----------
    def _update_device_specific_tabs(self):
        self._update_dmm_panel()
        self._update_smu_panel()
        self._update_fgen_panel()
