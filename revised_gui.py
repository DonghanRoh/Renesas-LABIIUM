    ...
    def dmm_show_label(self):
        try:
            ...
            if not self._is_supported_dmm(idn):
                messagebox.showinfo("Not a supported DMM", "현재 활성 장비가 지원 대상 DMM(MODEL 2000, 34410A, 34461A, 34465A)이 아닙니다.")
                return
            t, n = self._active_type_num()
            if not t:
                messagebox.showinfo("Missing Type", "Devices 표에서 Type을 선택해 주세요 (예: mm).")
                return
            label_text = (t.upper() + (f" {n}" if n and n != "No Number" else "")).strip()
            # SCPI 안전 따옴표 처리
            msg = label_text.replace("'", "''")

            sequences = [
                [f"DISP:TEXT:STAT ON", f"DISP:TEXT '{msg}'"],                # Keysight 계열 일반
                [f"DISPlay:TEXT:STATe ON", f"DISPlay:TEXT '{msg}'"],
                [f"DISP:TEXT '{msg}'"],
                [f"SYST:DISP:TEXT '{msg}'"],
                [f"DISP:WIND:TEXT '{msg}'"],                                  # Keithley 호환 시도
